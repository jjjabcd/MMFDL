...
class comModel(nn.Module):
    def __init__(self, args):
        ...
        # use vocab_size if provided, else fall back to num_features_smi (but this typically fails if they differ)
        self.vocab_size = args.get('vocab_size', self.num_features_smi) 
        ...
        # Use self.vocab_size for the embedding layer creation
        self.encoder = make_model(self.vocab_size, self.num_layer, self.hidden_dim, self.output_dim, self.num_heads, self.dropout)
    ...
    def get_embeddings(self, encodedSmi, encodedSmi_mask, ecfp, x, edge_index, batch):
        ...
        임베딩 추출을 위한 메서드
        Returns:
            smi_embedding: SMILES 임베딩 (batch_size, hidden_dim)
            ep_embedding: ECFP 임베딩 (batch_size, output_dim)
            gc_embedding: Graph 임베딩 (batch_size, 2*num_features_x)
        """
        smi_mask = encodedSmi_mask.unsqueeze(1)
        smi_encoded = self.encoder(encodedSmi, smi_mask)
        smi_encoded = smi_encoded.view(smi_encoded.shape[0], smi_encoded.shape[2], -1)
        smi_ln = self.smi_fc1(smi_encoded)
        smi_ln = smi_ln.view(smi_ln.shape[0], -1)
        smi_embedding = self.smi_fc2(smi_ln)  # (batch_size, hidden_dim) -> (batch_size, n_output)이지만, 실제로는 hidden_dim 차원의 중간 표현을 원함
        # forward와 동일하게 처리하되, embedding으로는 smi_ln을 사용
        smi_embedding = smi_ln  # (batch_size, hidden_dim)
    
        # ECFP를 GRU에 맞게 reshape
        if ecfp.dim() == 2:
